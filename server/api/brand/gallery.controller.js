...
function getresult(req, res) {
  ...
  _gallery2.default.findOne({ event_id: req.body.id }, function (err, orders) {
    if (err) {
...
function destroy(req, res) {
  Links.findByIdAsync(req.params.id).then(handleEntityNotFound(res)).then(removeEntity(res)).catch(handleError(res));
}
